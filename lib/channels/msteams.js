...
export function createMSTeamsChannel(config, log) {
    if (!config.enabled)
        return undefined;
    ...
    return {
        ...
        async send(chatId, text) {
            if (!appId || !appPassword)
                throw new Error('msteams: 发送需要 appId/appPassword');
            const conv = conversations.get(chatId);
            if (!conv?.serviceUrl)
                throw new Error('msteams: 缺少会话的 serviceUrl（等待对方先发消息）');
            ...
            if (!tokenData.access_token)
                throw new Error('msteams: 获取 Bot Framework token 失败');
            ...
            if (!res.ok)
                throw new Error(`msteams send: HTTP ${res.status}`);
        },
        setMessageHandler(h) {
            handler = h;
        },
