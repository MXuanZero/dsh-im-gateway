...
export class ImGateway {
    ...
    async listWorkspaces() {
        ...
        const lines = [`📁 工作区（${rows.length} 个）：`];
        for (const [cwd, info] of rows.slice(0, 20)) {
            lines.push(`• ${cwd}（${info.count} 会话，最近 ${relTime(info.last)}）`);
        }
        lines.push('用 /workspace <路径> 切换；/sessions <路径> 查看会话。');
        return lines.join('\n');
    }
    /** 列出会话（可按工作区过滤）。 */
    async listSessions(workspace) {
        ...
        const lines = [
            `📋 会话${workspace ? `（${workspace}）` : ''}：${filtered.length} 个${filtered.length > 20 ? `，显示前 20` : ''}`,
        ];
        top.forEach((r, i) => {
            const title = titles[i]?.ok ? (titles[i].value?.title ?? '') : '';
            const live = r.live ? '🟢' : '💤';
            const cwd = r.header.cwd ? ` · 📁 ${r.header.cwd.length > 36 ? `…${r.header.cwd.slice(-35)}` : r.header.cwd}` : '';
            lines.push(`[${i + 1}] ${r.header.id} ${title ? `「${title.slice(0, 30)}」` : ''}${r.header.createdAt ? ` · ${relTime(r.header.createdAt)}` : ''} ${live}${cwd}`);
        });
