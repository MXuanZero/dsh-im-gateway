...
export class QuestionBroker {
    pending = new Map();
    recent = new Map();
    hasPending(sessionId) { return this.pending.has(sessionId); }
    questionsFor(sessionId) { return this.pending.get(sessionId)?.questions; }
    wait(sessionId, questions, timeoutMs, signal) {
        this.pruneRecent();
        ...
        return new Promise((resolve) => {
            ...
            timer.unref?.();
            const entry = { questions, resolve: settle, timer, ...(signal ? { signal } : {}) };
            if (signal) {
    ...
    finishFromWeb(sessionId, answer) {
        const entry = this.pending.get(sessionId);
        if (!entry)
            return false;
    ...
    cancel(sessionId) { this.pending.get(sessionId)?.resolve({ kind: 'cancelled' }); }
    dispose() { for (const id of [...this.pending.keys()])
        this.cancel(id); this.recent.clear(); }
    remember(id, questions, message) {
...
export function formatQuestionPrompt(questions, timeoutSecs) {
    ...
    questions.forEach((question, index) => {
        ...
        if (question.multiSelect === true)
            lines.push('（可多选，用逗号分隔，如 1,3）');
        else if ((question.options?.length ?? 0) === 0)
            lines.push('（直接回复你的答案）');
        lines.push('');
    });
    if (questions.length === 1)
        lines.push('回复选项编号/文字；也可以直接回复自定义答案。');
    else
        lines.push('请每行回答一个问题，格式为「问题序号: 答案」，例如：', '1: 2', '2: 1,3');
    return lines.join('\n').trim();
}
export function formatAnswerSummary(questions, answer) {
...
export function parseQuestionReply(questions, rawText) {
    ...
    for (const line of lines) {
        ...
        if (index >= 0 && index < questions.length)
            values.set(index, match[2].trim());
    }
    if (values.size === 0 && lines.length === questions.length)
        lines.forEach((line, index) => values.set(index, line));
    if (values.size !== questions.length)
        return { ok: false, error: `请回答全部 ${questions.length} 个问题，每行使用「问题序号: 答案」，例如「1: 2」。` };
    const answers = [];
    for (let index = 0; index < questions.length; index += 1) {
        const parsed = parseOne(questions[index], values.get(index));
        if (!parsed.ok)
            return { ok: false, error: `问题 ${index + 1}：${parsed.error}` };
        answers.push(parsed.answer);
    }
    return { ok: true, answer: { answers } };
}
function looksLikeExplicitReply(questions, rawText) {
    const text = rawText.trim();
    if (questions.length > 1)
...
function parseOne(question, text) {
    const value = text.trim();
    if (value === '')
        return { ok: false, error: '答案不能为空。' };
    const options = question.options ?? [];
    if (options.length === 0)
        return { ok: true, answer: { id: question.id, selected: [], custom: value } };
    const labelAt = (token) => {
