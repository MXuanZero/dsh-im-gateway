...
`;
    ...
    const badgeFor = (ch) => {
      if (!ch.running) return { cls: "off", text: "未连接" };
      if (ch.status === "等待扫码" || ch.status === "登录中" || ch.loginUrl)
        return { cls: "wait", text: "等待扫码" };
      if (ch.status === "已登录" || ch.status === "已连接" || ch.status === "运行中" || ch.status === "轮询中" || ch.status === "已启动")
        return { cls: "on", text: "已连接" };
      if (ch.status.includes("错误") || ch.status.includes("失败"))
        return { cls: "err", text: "异常" };
      return { cls: "wait", text: ch.status };
    };
    ...
    function ChannelCard({ ch, busy, expanded, onToggleExpand, onAction }) {
      ...
      // 头部行：真实品牌图标（favicon 服务，加载失败回退 emoji）
      const iconUrl = ch.iconDomain ? `https://icons.duckduckgo.com/ip3/${ch.iconDomain}.ico` : null;
      children.push(
      ...
      // 未连接：按 kind 渲染
      if (ch.kind === "stub") {
        children.push(h("div", { key: "hint", className: "imgw-hint" }, ch.hint));
        children.push(
