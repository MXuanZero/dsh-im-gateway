...
export function createTwitchChannel(config: TwitchChannelConfig, log: (line: string) => void): ChannelAdapter | undefined {
  const botName = config.botName ?? process.env.DSH_TWITCH_BOT_NAME
  const token = config.token ?? process.env.DSH_TWITCH_TOKEN
  if (!botName || !token) return undefined
