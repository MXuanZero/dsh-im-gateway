...
export function createNostrChannel(config: NostrChannelConfig, log: (line: string) => void): ChannelAdapter | undefined {
  ...
  if (!privateKey || relays.length === 0) return undefined
  ...
  adapter = {
    ...
    async start() {
      stopped = false
      let noble: typeof import('@noble/curves/secp256k1')
      try {
      ...
      ws.onmessage = (ev) => {
        ...
        if (type === 'EVENT' && rest.length > 0) {
          const event = rest[0] as { id?: string; pubkey?: string; content?: string }
          if (!event.id || seen.has(event.id) || !event.pubkey || !event.content) return
          seen.add(event.id)
          try {
