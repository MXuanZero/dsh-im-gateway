...
export function createWebChatChannel(config: WebChatChannelConfig, log: (line: string) => void): ChannelAdapter | undefined {
    ...
    async start() {
      ...
      server = createServer((req, res) => {
        ...
        if (url.pathname === '/api/send' && req.method === 'POST') {
          ...
          req.on('end', () => {
            try {
              const data = JSON.parse(body) as { text?: string; token?: string }
